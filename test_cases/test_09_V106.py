...
class OpenLive(unittest.TestCase):
    ...
    # 读取excel当中的headers，得到字典
        ...
        try:
            ...
            self.excel_handler.write(config.data_path, "V107", test_data["case_id"] + 1, 9, "测试通过")
        except AssertionError as e:
            self.logger.error(e)
            # 手动抛出异常，否则测试用例自动通过
            self.excel_handler.write(config.data_path, "V107", test_data["case_id"] + 1, 9, "测试失败")
            raise e
