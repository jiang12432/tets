...
class OpenLive(unittest.TestCase):
    ...
    data = excel_handler.read("livemothd")
    # 数据放到前置条件当中
    ...
    def setUp(self) -> None:
        ...
        self.token1 = save_data()
        room = Live()
        # 用户加入直播间，并获取直播间的roomid
        self.roomdd = room.joinRoom()
    ...
    # 读取excel当中的headers，得到字典
        ...
        res = self.req.visit(url=config.host + test_data["url"],
                             ...
                             #  将参数中的#roomid#替换成直播间的roomid
                             json=utils.TypeConversionStrUidToInt("\"#roomid#\"", test_data["data"], self.roomdd),
                             headers=headers)
        print(res)
        try:
            self.assertEqual(res["code"], test_data["expected"])
            self.excel_handler.write(config.data_path, "livemothd", test_data["case_id"] + 1, 9, "测试通过")
        except AssertionError as e:
            self.logger.error(e)
            print(res)
            # 手动抛出异常，否则测试用例自动通过
            self.excel_handler.write(config.data_path, "livemothd", test_data["case_id"] + 1, 9, "测试失败")
            raise e
